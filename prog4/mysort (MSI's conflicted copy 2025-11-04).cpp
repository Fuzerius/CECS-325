    ...
    for (int pass = 0; pass < size - 1; pass++) {
        for (int i = 0; i < size - 1 - pass; i++) {
            if (arr[i] > arr[i + 1]) {
                ...
                // Increment local swap counter
                localSwaps++;
            }
        }
    }
    
    // Store the local swap count for this thread and update global counter
    // Only lock once at the end instead of for every swap
    locker.lock();
    totalSwaps += localSwaps;  // Add all local swaps to global counter at once
